...
let wire1 = input1.split(",");
let wire2 = input2.split(",");

const wire = input => {
  let wire = [{ x: 0, y: 0 }];
  let previousVector = { ...wire[wire.length - 1] };
  let vector = {};

  for (let i = 0; i < input.length; i++) {
    let num = Number(input[i].substring(1));
    if (input[i][0] === "R") {
      vector = { ...previousVector };
      vector.x = previousVector.x + num;
      vector.y = previousVector.y;
    ...
    if (input[i][0] === "U") {
      vector = { ...previousVector };
      vector.y = previousVector.y + num;
      vector.x = previousVector.x;
      previousVector = vector;
      wire.push(vector);
    }
    if (input[i][0] === "D") {
...
const grid1 = wire(wire1);
const grid2 = wire(wire2);
...
//console.log('interse',intersects(grid1, grid2));
function coordinates(wire1, wire2) {
  let intersections = [];
  ...
  for (let i = 0; i < wire1.length - 1; i++) {
    for (let j = 0; j < wire2.length - 1; j++) {
      let x1 = wire1[i].x;
      let y1 = wire1[i].y;
      let x2 = wire1[i + 1] ? wire1[i + 1].x : "c";
      let y2 = wire1[i + 1] ? wire1[i + 1].y : "d";
      let x3 = wire2[j].x;
      let y3 = wire2[j].y;
      let x4 = wire2[j + 1] ? wire2[j + 1].x : "r";
      let y4 = wire2[j + 1] ? wire2[j + 1].y : "s";
      const result = intersects(x1, y1, x2, y2, x3, y3, x4, y4);
      if (result) {
...
//console.log("coor", coordinates(grid1, grid2));

function manhattanDistance(intersections) {
  let distance = "distance";
  for (coord of intersections) {
    coord[distance] = Math.abs(coord.x) + Math.abs(coord.y);
  }
  let distanceArray = intersections.map(coordinate => {
    return coordinate.distance;
  });
  let manhattanDistance = Math.min(...distanceArray);
  return manhattanDistance;
}

console.log("manhattanDistance", manhattanDistance(coordinates(grid1, grid2)));
